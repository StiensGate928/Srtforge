...
class InspectorProbeWorker(QtCore.QThread):
    """Background ffprobe worker for the right-side Inspector panel."""

    resultReady = QtCore.Signal(int, dict)  # request_id, payload
    ...
    def run(self) -> None:  # noqa: D401 - Qt override
        ...
        try:
            stat = path.stat()
            payload["exists"] = True
            payload["size_bytes"] = int(getattr(stat, "st_size", 0) or 0)
            payload["mtime"] = float(getattr(stat, "st_mtime", 0.0) or 0.0)
        except Exception as exc:
            payload["error"] = str(exc)
            self.resultReady.emit(self._request_id, payload)
            return

        # ffprobe (format + first video stream)
        try:
            meta = _probe_media_metadata_ffprobe(path, self._ffprobe_bin)
            payload.update(meta)
        except Exception as exc:
            payload["probe_error"] = str(exc)

        # Audio streams (reuse the pipeline's FFmpegTooling parser)
        try:
            tooling = FFmpegTooling(ffprobe_bin=str(self._ffprobe_bin or "ffprobe"))
            streams = tooling.probe_audio_streams(path)
            payload["audio_streams"] = [
                {
                    "index": s.index,
                    "codec": s.codec_name,
                    "language": s.language,
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        # Right-side Inspector panel state
        self._inspector_request_id: int = 0
        self._inspector_worker: Optional[InspectorProbeWorker] = None
        self._inspector_media_path: Optional[Path] = None
    ...
    def _build_ui(self) -> None:
        ...
        # --- Right-side Inspector panel (fills unused horizontal space) -----
        self.inspector_panel = QtWidgets.QFrame()
        self.inspector_panel.setObjectName("InspectorPanel")
        insp_layout = QtWidgets.QVBoxLayout(self.inspector_panel)
        insp_layout.setContentsMargins(12, 0, 0, 0)
        insp_layout.setSpacing(10)

        insp_title_row = QtWidgets.QHBoxLayout()
        insp_title = QtWidgets.QLabel("Inspector")
        insp_title.setObjectName("InspectorTitle")
        insp_title_row.addWidget(insp_title)
        insp_title_row.addStretch()

        self.inspector_refresh_btn = QtWidgets.QToolButton()
        self.inspector_refresh_btn.setObjectName("InspectorRefresh")
        self.inspector_refresh_btn.setCursor(pointer_cursor)
        self.inspector_refresh_btn.setAutoRaise(True)
        self.inspector_refresh_btn.setToolTip("Refresh file details")
        self.inspector_refresh_btn.setIcon(
            self.style().standardIcon(QtWidgets.QStyle.StandardPixmap.SP_BrowserReload)
        )
        self.inspector_refresh_btn.clicked.connect(self._refresh_inspector)
        insp_title_row.addWidget(self.inspector_refresh_btn)

        insp_layout.addLayout(insp_title_row)
        ...
        file_tab_layout.setContentsMargins(0, 8, 0, 0)
        file_tab_layout.setSpacing(10)

        # Full path row (read-only) + actions
        path_row = QtWidgets.QHBoxLayout()
        self.inspector_path_edit = QtWidgets.QLineEdit()
        self.inspector_path_edit.setObjectName("InspectorPath")
        self.inspector_path_edit.setReadOnly(True)
        self.inspector_path_edit.setPlaceholderText("Select a file in the queue")
        path_row.addWidget(self.inspector_path_edit, 1)

        self.inspector_copy_btn = QtWidgets.QToolButton()
        self.inspector_copy_btn.setObjectName("InspectorCopyPath")
        self.inspector_copy_btn.setCursor(pointer_cursor)
        self.inspector_copy_btn.setAutoRaise(True)
        self.inspector_copy_btn.setToolTip("Copy full path")
        copy_icon = QtGui.QIcon.fromTheme("edit-copy")
        if not copy_icon.isNull():
            self.inspector_copy_btn.setIcon(copy_icon)
        else:
            # Unicode fallback (keeps the UI usable even without a theme icon).
            self.inspector_copy_btn.setText("⧉")
            self.inspector_copy_btn.setToolButtonStyle(
                QtCore.Qt.ToolButtonStyle.ToolButtonTextOnly
            )
        self.inspector_copy_btn.clicked.connect(self._copy_inspector_path)
        path_row.addWidget(self.inspector_copy_btn)

        self.inspector_open_folder_btn = QtWidgets.QToolButton()
        self.inspector_open_folder_btn.setObjectName("InspectorOpenFolder")
        self.inspector_open_folder_btn.setCursor(pointer_cursor)
        self.inspector_open_folder_btn.setAutoRaise(True)
        self.inspector_open_folder_btn.setToolTip("Open containing folder")
        self.inspector_open_folder_btn.setIcon(
            self.style().standardIcon(QtWidgets.QStyle.StandardPixmap.SP_DirOpenIcon)
        )
        self.inspector_open_folder_btn.clicked.connect(self._open_inspector_folder)
        path_row.addWidget(self.inspector_open_folder_btn)

        file_tab_layout.addLayout(path_row)
        ...
        meta_form.setFormAlignment(QtCore.Qt.AlignTop)

        self.inspector_container_value = QtWidgets.QLabel("–")
        self.inspector_size_value = QtWidgets.QLabel("–")
        self.inspector_mtime_value = QtWidgets.QLabel("–")
        self.inspector_duration_value = QtWidgets.QLabel("–")
        self.inspector_video_value = QtWidgets.QLabel("–")

        meta_form.addRow("Container", self.inspector_container_value)
        meta_form.addRow("Size", self.inspector_size_value)
        meta_form.addRow("Modified", self.inspector_mtime_value)
        meta_form.addRow("Duration", self.inspector_duration_value)
        meta_form.addRow("Video", self.inspector_video_value)

        file_tab_layout.addLayout(meta_form)

        # Audio streams list
        audio_title = QtWidgets.QLabel("Audio streams")
        audio_title.setObjectName("InspectorSectionTitle")
        file_tab_layout.addWidget(audio_title)

        self.inspector_audio_list = QtWidgets.QTreeWidget()
        self.inspector_audio_list.setObjectName("InspectorAudioList")
        self.inspector_audio_list.setHeaderLabels(["#", "Lang", "Codec", "Ch", "Hz", "Layout"])
        self.inspector_audio_list.setRootIsDecorated(False)
        self.inspector_audio_list.setItemsExpandable(False)
        self.inspector_audio_list.setIndentation(0)
        self.inspector_audio_list.setUniformRowHeights(True)
        self.inspector_audio_list.setEditTriggers(
            QtWidgets.QAbstractItemView.EditTrigger.NoEditTriggers
        )
        self.inspector_audio_list.setSelectionMode(
            QtWidgets.QAbstractItemView.SelectionMode.NoSelection
        )
        self.inspector_audio_list.setMinimumHeight(140)
        audio_header = self.inspector_audio_list.header()
        audio_header.setStretchLastSection(True)
        audio_header.setSectionResizeMode(QtWidgets.QHeaderView.ResizeMode.ResizeToContents)
        file_tab_layout.addWidget(self.inspector_audio_list, 1)

        self.inspector_error_label = QtWidgets.QLabel("")
        self.inspector_error_label.setObjectName("InspectorError")
        self.inspector_error_label.setWordWrap(True)
        self.inspector_error_label.setVisible(False)
        file_tab_layout.addWidget(self.inspector_error_label)

        file_tab_layout.addStretch(1)

        self.inspector_tabs.addTab(file_tab, "File")
        insp_layout.addWidget(self.inspector_tabs, 1)

        # Place queue + inspector side-by-side so the Output column reaches
        # the edge of the queue viewport without stretching.
        self.queue_splitter = QtWidgets.QSplitter(QtCore.Qt.Orientation.Horizontal)
        self.queue_splitter.setObjectName("QueueSplitter")
        self.queue_splitter.setChildrenCollapsible(False)
        self.queue_splitter.setHandleWidth(1)
        self.queue_splitter.addWidget(self.queue_list)
        self.queue_splitter.addWidget(self.inspector_panel)
        self.queue_splitter.setStretchFactor(0, 3)
        self.queue_splitter.setStretchFactor(1, 1)
        # Size hint: keep the queue columns snug (no dead space) while still
        # giving the Inspector a usable width.
        try:
            self.inspector_panel.setMinimumWidth(280)
            header = self.queue_list.header()
            columns_total = sum(header.sectionSize(i) for i in range(header.count()))
            # + scrollbar + some padding
            columns_total += 32

            available = max(960, int(self.width() or 0))
            right_min = 320
            left = int(columns_total)
            if left + right_min > available:
                left = max(620, available - right_min)
            right = max(260, available - left)
            self.queue_splitter.setSizes([left, right])
        except Exception:
            # Worst case: Qt will auto-allocate.
            pass

        # Update inspector whenever a row is selected.
    ...
    def _apply_styles(self) -> None:
        ...
        # ---- Inspector panel (right side of the queue splitter) --------------
        if self._dark_mode:
            insp_divider = "rgba(148, 163, 184, 0.25)"
            insp_error = "#FCA5A5"
            insp_section = "rgba(226, 232, 240, 0.75)"
            insp_list_bg = "#020617"
            insp_list_border = "#111827"
        else:
            insp_divider = "#E2E8F0"
            insp_error = "#DC2626"
            insp_section = "#475569"
            insp_list_bg = "#FFFFFF"
            insp_list_border = "#E2E8F0"

        custom += f"""
        /* Queue splitter handle as a subtle divider */
        QSplitter#QueueSplitter::handle {{
            background-color: {insp_divider};
        }}
        ...
        QLabel#InspectorError {{
            color: {insp_error};
        }}

        QTreeWidget#InspectorAudioList {{
            background-color: {insp_list_bg};
            border: 1px solid {insp_list_border};
            border-radius: 10px;
        }}
    ...
    def _update_start_state(self) -> None:
        ...
        if hasattr(self, "queue_stack"):
            self.queue_stack.setCurrentWidget(
                self.queue_splitter if has_items else self.queue_placeholder
            )

        # Keep the inspector in a sensible state when the queue is empty.
        if not has_items:
    ...
    def _reset_inspector(self) -> None:
        """Reset the right-side Inspector UI to its empty state."""

        self._inspector_media_path = None

        if hasattr(self, "inspector_path_edit"):
            self.inspector_path_edit.setText("")
            self.inspector_path_edit.setPlaceholderText("Select a file in the queue")

        for attr in (
            "inspector_container_value",
            "inspector_size_value",
            "inspector_mtime_value",
            "inspector_duration_value",
            "inspector_video_value",
        ):
            if hasattr(self, attr):
                getattr(self, attr).setText("–")

        if hasattr(self, "inspector_audio_list"):
            self.inspector_audio_list.clear()

        if hasattr(self, "inspector_error_label"):
            self.inspector_error_label.setVisible(False)
            self.inspector_error_label.setText("")

        # Disable actions until a real file is selected.
        for btn_name in (
            "inspector_copy_btn",
            "inspector_open_folder_btn",
            "inspector_refresh_btn",
        ):
            if hasattr(self, btn_name):
                getattr(self, btn_name).setEnabled(False)

    def _set_inspector_loading(self, path: Path) -> None:
        """Show a lightweight loading state while ffprobe runs in the background."""

        if hasattr(self, "inspector_path_edit"):
            self.inspector_path_edit.setText(str(path))
        ...
        for attr in (
            ...
            if hasattr(self, attr):
                getattr(self, attr).setText("Loading…")

        if hasattr(self, "inspector_audio_list"):
            self.inspector_audio_list.clear()
            item = QtWidgets.QTreeWidgetItem(["", "", "Loading…", "", "", ""])
            self.inspector_audio_list.addTopLevelItem(item)

        if hasattr(self, "inspector_error_label"):
            self.inspector_error_label.setVisible(False)
            self.inspector_error_label.setText("")

        # Enable actions for the selected file.
        for btn_name in (
            "inspector_copy_btn",
            "inspector_open_folder_btn",
            "inspector_refresh_btn",
    ...
    def _on_queue_selection_changed(self) -> None:
        items = self.queue_list.selectedItems() if hasattr(self, "queue_list") else []
        if not items:
            self._reset_inspector()
            return
        if len(items) != 1:
            ...
            if hasattr(self, "inspector_error_label"):
                self.inspector_error_label.setText(
                    f"{len(items)} items selected — select a single file to inspect."
                )
                self.inspector_error_label.setVisible(True)
            return

        raw = items[0].data(0, QtCore.Qt.ItemDataRole.UserRole)
        if not raw:
            self._reset_inspector()
            return

        path = Path(str(raw))
        self._inspector_media_path = path
        self._set_inspector_loading(path)
        self._start_inspector_probe(path)

    def _refresh_inspector(self) -> None:
        """Manually refresh the Inspector for the currently selected row."""
        path = getattr(self, "_inspector_media_path", None)
        if not path:
            return
        self._set_inspector_loading(path)
        self._start_inspector_probe(path)

    def _copy_inspector_path(self) -> None:
        path = getattr(self, "_inspector_media_path", None)
        if not path:
            return
        try:
    ...
    def _open_inspector_folder(self) -> None:
        path = getattr(self, "_inspector_media_path", None)
        if not path:
            return
        try:
    ...
    def _start_inspector_probe(self, path: Path) -> None:
        """Kick off an async ffprobe + audio probe run."""

        self._inspector_request_id += 1
        request_id = self._inspector_request_id

        # Let any previous worker finish; we just ignore stale results.
        try:
            if self._inspector_worker and self._inspector_worker.isRunning():
                self._inspector_worker.requestInterruption()
        except Exception:
            pass

        ffprobe_bin = self.ffmpeg_paths.ffprobe if self.ffmpeg_paths else None
        worker = InspectorProbeWorker(request_id, path, ffprobe_bin, parent=self)
        worker.resultReady.connect(self._on_inspector_probe_result)
        worker.finished.connect(worker.deleteLater)
        self._inspector_worker = worker
        worker.start()

    def _on_inspector_probe_result(self, request_id: int, payload: dict) -> None:
        """Receive background probe results and update the Inspector UI."""

        if request_id != getattr(self, "_inspector_request_id", 0):
            return

        # Keep a strong reference to the path shown.
        try:
            self._inspector_media_path = Path(str(payload.get("path") or ""))
        except Exception:
            self._inspector_media_path = None

        # Path text
        if hasattr(self, "inspector_path_edit"):
            self.inspector_path_edit.setText(str(payload.get("path", "")))

        # Errors
        error_text = payload.get("error") or payload.get("probe_error")
        if error_text and hasattr(self, "inspector_error_label"):
            self.inspector_error_label.setText(str(error_text))
            self.inspector_error_label.setVisible(True)
        elif hasattr(self, "inspector_error_label"):
            self.inspector_error_label.setVisible(False)
            self.inspector_error_label.setText("")
        ...
        try:
            ...
            fmt = str(payload.get("format_name") or "").strip()
            if ext and fmt and fmt.lower() not in {ext.lower()}:
                container = f"{ext} ({fmt})"
            elif ext:
                container = ext
            elif fmt:
            ...
            container = "–"
        if hasattr(self, "inspector_container_value"):
            self.inspector_container_value.setText(container or "–")

        # File size + modified time
        if hasattr(self, "inspector_size_value"):
            self.inspector_size_value.setText(_format_bytes(int(payload.get("size_bytes") or 0)))
        if hasattr(self, "inspector_mtime_value"):
            ts = float(payload.get("mtime") or 0.0)
            self.inspector_mtime_value.setText(_format_timestamp(ts) if ts else "–")

        # Duration
        if hasattr(self, "inspector_duration_value"):
            self.inspector_duration_value.setText(_format_hms(float(payload.get("duration_s") or 0.0)))

        # Video info
        vid_parts: list[str] = []
        w = payload.get("video_width")
        h = payload.get("video_height")
        fps = float(payload.get("video_fps") or 0.0)
        codec = str(payload.get("video_codec") or "").strip()
        if w and h:
            vid_parts.append(f"{w}×{h}")
        if fps > 0:
            vid_parts.append(f"{fps:.3f} fps")
        if codec:
            vid_parts.append(codec)
        video_str = " · ".join(vid_parts) if vid_parts else "–"
        if hasattr(self, "inspector_video_value"):
            self.inspector_video_value.setText(video_str)

        # Audio list
        if hasattr(self, "inspector_audio_list"):
            self.inspector_audio_list.clear()

            streams = payload.get("audio_streams") or []
            for s in streams:
                idx = str(s.get("index") if s.get("index") is not None else "")
                lang = str(s.get("language") or "–")
                codec_s = str(s.get("codec") or "–")
                ch = str(s.get("channels") if s.get("channels") is not None else "–")
                hz = str(s.get("sample_rate") if s.get("sample_rate") is not None else "–")
                layout = str(s.get("layout") or "–")
                item = QtWidgets.QTreeWidgetItem([idx, lang, codec_s, ch, hz, layout])
                self.inspector_audio_list.addTopLevelItem(item)

            if not streams:
                self.inspector_audio_list.addTopLevelItem(
                    QtWidgets.QTreeWidgetItem(["", "", "No audio streams detected", "", "", ""])
                )

            audio_error = payload.get("audio_error")
            if audio_error and hasattr(self, "inspector_error_label"):
                # Non-fatal: show under list.
                self.inspector_error_label.setText(str(audio_error))
                self.inspector_error_label.setVisible(True)
...
def _parse_ffprobe_rate(value: str) -> float:
    """Parse ffprobe frame-rate strings like '24000/1001' into a float."""
...
def _probe_media_metadata_ffprobe(media: Path, ffprobe_bin: Optional[Path]) -> dict:
    """Return container + basic video info for ``media`` via ffprobe.

    The returned dict is intentionally "flat" so it can be passed through Qt
    signals without custom types.
    """
    ...
    command = [
        ...
        "v:0",
        "-show_entries",
        "format=format_name,format_long_name,duration:stream=codec_name,width,height,avg_frame_rate,r_frame_rate",
        "-of",
        "json",
        str(media),
    ]
    ...
    if result.returncode != 0:
        # Keep the UI resilient; upstream caller can surface stderr if desired.
    ...
    if streams:
        stream = streams[0] or {}
        out["video_codec"] = stream.get("codec_name", "") or ""
        try:
            out["video_width"] = int(stream.get("width")) if stream.get("width") else None
        except Exception:
            out["video_width"] = None
        try:
            out["video_height"] = int(stream.get("height")) if stream.get("height") else None
        except Exception:
            out["video_height"] = None
        fps = _parse_ffprobe_rate(str(stream.get("avg_frame_rate") or ""))
        if fps <= 0:
            fps = _parse_ffprobe_rate(str(stream.get("r_frame_rate") or ""))
        out["video_fps"] = float(fps or 0.0)

    return out


def _format_bytes(num_bytes: int) -> str:
    """Human-friendly byte formatting used by the Inspector."""
